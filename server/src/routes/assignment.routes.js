...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!video.isPublished)
      return res.status(400).json({ message: 'Publish the video before assigning it to learners' });
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      assignments: availableAssignments.map((assignment) => ({
        ...
        progress: byAssignment.get(assignment.id) || null,
      })),
    });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [video, questions, progress] = await Promise.all([
      Video.findById(assignment.videoId),
      Question.find({ videoId: assignment.videoId }).sort({ timestampSeconds: 1 }),
      Progress.findOne({ assignmentId: assignment.id }),
    ]);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    const assignment = await learnerAssignment(req.params.assignmentId, req.user.id);
    if (!assignment) return res.status(404).json({ message: 'Assignment not found' });
    const progress = await Progress.findOneAndUpdate({ assignmentId: assignment.id }, progressInput.parse(req.body), {
      new: true,
      upsert: true,
      runValidators: true,
    });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const response = await Response.findOneAndUpdate(
      { assignmentId: assignment.id, questionId: question.id },
      { answer: input.answer },
      { new: true, upsert: true, runValidators: true },
    );
    const progress = await Progress.findOneAndUpdate(
      { assignmentId: assignment.id },
      { $addToSet: { answeredQuestionIds: question.id } },
